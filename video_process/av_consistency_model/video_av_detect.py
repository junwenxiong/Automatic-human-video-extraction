...
def main(arg):
    ...
    talknet = talkNet()
    talknet.loadParameters(args.pretrainModel)
    ...
    for video_path in video_lists:
        ...
        save_path = video_dir.replace("TikTokDownload", "TikTokDownload/Temp_dir")
...
if __name__ == "__main__":
    ...
    save_json_path = "/cpfs/user/xiongjunwen/workspace/Scraper/TikTokDownload/Download/douyin/filter_info/AV_Consistency.json"
    with open(
        "/cpfs/user/xiongjunwen/workspace/Scraper/TikTokDownload/Download/douyin/filter_info/filter_videos_by_pose_detection.json",
        "r",
    ) as f:
        data = json.load(f)
    ...
    with mp.Pool(num_threads) as pool:
        results = pool.map(
            main, zip(range(num_threads), gpu_ids[:num_threads], video_list)
        )
    results_dict = {}
    for p in results:
